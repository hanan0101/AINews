...
UI_HTML_FILE = FRONTEND_DIR / "UI.html"
...
def extract_ui_styles() -> str:
    try:
        html = UI_HTML_FILE.read_text(encoding="utf-8")
    except Exception:
        return ""
...
def build_preview_pdf_document(preview_html: str, width: float, height: float, origin: str, direction: str = "rtl", scale: float = 1.0) -> str:
    """Wrap the live preview HTML in a clean document for Playwright export."""
    css = extract_ui_styles()
...
def export_preview_pdf_bytes(preview_html: str, width: float, height: float, origin: str, direction: str = "rtl", scale=None, profile: str = "") -> bytes:
    ...
    document_html = build_preview_pdf_document(preview_html, base_width, base_height, origin, direction, pdf_scale)
    trace(

...
AI_UPDATES_SEARXNG_TOOL_QUERY_LIMIT = max(1, env_int("AI_UPDATES_SEARXNG_TOOL_QUERY_LIMIT", "6"))
...
def exa_tool_update_script_rows(*, single: bool = False) -> tuple[list[dict], list[dict]]:
    ...
    rows = []
    official_tools = tools[:6] if single else tools[:AI_UPDATES_OFFICIAL_TOOL_QUERY_LIMIT]
    for tool in official_tools:
        ...
        query = f'site:{official_domain} "{tool_name}" update' if official_domain else f'"{tool_name}" update'
        rows.append({
            "query": query,
    ...
    broad_queries = list(EXA_PRODUCT_UPDATE_BROAD_QUERIES[:3] if single else EXA_PRODUCT_UPDATE_BROAD_QUERIES)
...
def discovery_rows(source: str, *, single: bool = False, target_hint: str = "") -> list[dict]:
    ...
    if source == "searxng":
        ...
        tool_limit = max(1, min(AI_UPDATES_TOOL_QUERY_ROTATION_POOL, searxng_limit))
        tools = load_monthly_tool_records(limit=tool_limit)
        rows = []
    ...
    if source == "exa":
        rows, tools = exa_tool_update_script_rows(single=single)
        LAST_DISCOVERY_META[source] = {
            "tool_count": len(tools),
            "official_tool_query_limit": AI_UPDATES_OFFICIAL_TOOL_QUERY_LIMIT,
            "tool_names": [tool.get("tool") for tool in tools],
...
def fetch_searxng_query_rows(rows: list[dict], *, exclude_items: list[dict] | None = None, single: bool = False) -> tuple[list[dict], dict]:
    ...
    def fetch_row(row: dict):
        ...
        params = {
            ...
            # SearXNG news fetch: prefer mainstream web engines for fresh
            # company announcements while keeping the existing weekly window.
            "engines": "brave,startpage,google,bing" if url_discovery_only else "google,bing,brave",
...
def exa_recent_verify_date_details(url: str, timeout: int = EXA_RECENT_PAGE_TIMEOUT):
    ...
    for source_name, date_value in sources:
        if date_value:
            return date_value, source_name, "", str(response.url)
    return None, "", "no_verified_date", str(response.url)
    ...
    def fetch_row(row: dict):
        ...
        def exa_request(query: str, *, num_results: int, start_published_date: str = "") -> tuple[list[dict], str, str]:
            ...
            payload = {
                ...
                "contents": {"text": False, "highlights": False} if script_style else {"text": True, "highlights": True},
            }
            if not script_style:
                payload["startPublishedDate"] = start_published_date or recency_cutoff_query_token()
                        ...
                        if is_official_url and raw_date_value:
                            ...
                            if not (EXA_RECENT_MAX_PAGES_PER_TOOL and pages_checked > EXA_RECENT_MAX_PAGES_PER_TOOL):
                                ...
                                if page_date and page_date >= cutoff:
                                    ...
                                    acceptance_reason = f"official_domain_update_like_{reason or 'no_verified_date'}"
                                    date_confidence = "low"
                            else:
                                rejection_audit.append({
                                    "title": title,
        ...
        if row.get("official_site") and not accepted_items and not script_style:
...
SAUDI_TRUSTED_MEDIA_DOMAINS = ("arabnews.com", "saudigazette.com.sa", "argaam.com", "reuters.com", "bloomberg.com")
SAUDI_TERMS_TRACKER = (
...
def fetch_news_candidates(*, exclude_items: list[dict] | None = None, target_hint: str = "", single: bool = False) -> tuple[list[dict], dict]:
    ...
    if not single and AI_UPDATES_GENERAL_NEWS_LAYER_ENABLED:
...
# Compatibility exports for older scripts. The implementation now lives in
# domain-specific fetching modules.
_COURSE_EXPORTS = {
    "COURSE_BAD_URL_TERMS",
    "COURSE_PAGE_TEXT_TERMS",
    "OPEN_WEB_COURSE_QUERIES",
    "COURSE_RAW_PLATFORM_CAP",
    "COURSE_MAJOR_PLATFORM_CAP",
    "COURSE_MAJOR_PLATFORM_DOMAINS",
    "COURSE_MAJOR_PLATFORM_NAMES",
    "COURSE_DIVERSITY_TARGET_DOMAINS",
    "DISCOVERED_PLATFORMS_FILE",
    "APP_STORE_DOMAINS",
    "COURSE_DIRECT_PATHS",
    ...
    "COURSE_PLATFORM_DISCOVERY_RE",
    "COURSE_AI_DISCOVERY_RE",
    "COURSE_INTENT_QUERY",
    "WORKFORCE_INTENT_QUERY",
    "COURSE_NON_FATAL_PAGE_STATUSES",
    ...
    "domain_matches",
    "clean_course_url",
    "course_platform_from_url",
    "load_discovered_platforms",
    "readable_platform_name",
    "most_common_discovered_path",
    "apply_discovered_platform_promotions",
    "record_discovered_course_platforms",
    "course_url_is_direct",
    "infer_course_level",
    "course_candidate_topic_key",
    "normalize_course_candidate",
    "diversify_course_candidates",
    "fetch_exa_course_query",
    "fetch_open_web_course_candidates",
    "rotated_course_queries",
    "course_query_batches",
    "save_course_query_rotation",
    "course_pool_platform_name",
    "is_major_course_platform",
    "load_last_course_platforms",
    "next_course_fetch_platform_domains",
    "build_targeted_course_queries",
    "course_platform_domain_allowed",
    "fetch_course_platform_page_text",
    "course_platform_page_title",
    "course_platform_direct_seed_result",
    "course_platform_discovery_queries",
    "score_course_platform_result",
    "fetch_exa_course_platform_raw",
    "fetch_course_platform_discovery",
    "add_course_pool_candidate",
    "fetch_course_candidates",
}
_FILM_EXPORTS = {
    "TMDB_AI_KEYWORD_IDS",
    "rotated_movie_pages",
    "fetch_movie_candidates",
}

def __getattr__(name: str):
    if name in _COURSE_EXPORTS:
        from backend.pipeline.fetching import courses as course_fetching
        return getattr(course_fetching, name)
    if name in _FILM_EXPORTS:
        from backend.pipeline.fetching import films as film_fetching
        return getattr(film_fetching, name)

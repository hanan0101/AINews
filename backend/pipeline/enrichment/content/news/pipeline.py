...
from backend.pipeline.enrichment.shared.logos import (
    NEWS_SOURCE_LOGO_KEYS,
    company_logo_candidates,
    first_verified_logo,
    logo_name_key,
)
from backend.pipeline.filtering.content.news.rules import save_news_memory
from backend.pipeline.filtering.content.courses.level_balancing import (
    COURSES_PER_LEVEL,
    NEWS_PER_LEVEL,
    build_level_bank,
...
def news_item_from_update(update: dict, index: int) -> dict:
    ...
    logo_for_card = first_verified_logo(logo_candidates)
    ...
    return {
        ...
        "logo": logo_for_card,
        "logo_size": 30,
        "logo_candidates": list(dict.fromkeys(str(url or "").strip() for url in logo_candidates if str(url or "").strip()))[:6],

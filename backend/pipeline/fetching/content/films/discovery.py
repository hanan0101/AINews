...
from backend.pipeline.fetching.fetch_utils import safe_print
...
def rotated_movie_pages(page_count: int) -> tuple[list[int], dict]:
    ...
    rotation = rotation_state("movie_page_rotation")
    start_page = int(rotation.get("next_page") or 1)
    if start_page < 1 or start_page > max_page:
        start_page = 1
    indices, next_cursor = rotation_window(start_page - 1, max_page, page_count)
    pages = [i + 1 for i in indices]
    next_page = next_cursor + 1
    new_rotation = save_rotation_state("movie_page_rotation", {

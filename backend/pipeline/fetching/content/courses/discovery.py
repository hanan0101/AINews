...
from backend.config.settings import (
    AI_UPDATES_COURSE_EXA_ALLOW_OUTSIDE_INCLUDE_DOMAINS,
...
from backend.pipeline.fetching.content.courses.bank import upsert_course_bank
...
def normalize_course_candidate(raw: dict, *, fetch_source: str, query: str = "") -> dict | None:
    ...
    level_result = classify_course_level_evidence(title, content, url)
    if level_result["level_confidence"] == "low":
        official_page_text, page_status = fetch_course_platform_page_text(url)
        if official_page_text:
            level_result = classify_course_level_evidence(title, official_page_text, url)
            if level_result["level_source"] != "unverified":
                level_result["level_source"] = f"official_page_{level_result['level_source'].removeprefix('official_')}"
        level_result["official_page_status"] = page_status
    item = {
        ...
        "level_confidence": level_result["level_confidence"],
        "level_source": level_result["level_source"],
        "level_evidence": level_result["level_evidence"],
...
def course_query_batches(query_bank: list[str], query_limit: int) -> tuple[list[list[str]], dict]:
    ...
    rotation = rotation_state("course_query_rotation")
...
def save_course_query_rotation(rotation: dict, attempted_queries: list[str], attempted_batches: int, found_count: int) -> dict:
    ...
    next_index = rotation_window(start, total, max(1, len(attempted_queries)))[1] if total else 0
    return save_rotation_state("course_query_rotation", {
        **rotation,
        "updated_at": utc_now().isoformat(),

...
from server_items import dedupe_store_items, normalize_item
...
from ai_update_pipeline import run_single_supporting_pipeline as run_ai_updates_single_supporting_pipeline
from ai_update_pipeline import run_single_update_pipeline as run_ai_updates_single_pipeline
...
def current_exclusion_items(store, section, target_id=None, extra_exclude=None):
    current = [
        item for item in visible_items(store, section)
...
def _single_item_refill_impl(store, section, item_id=None, index=None, action="replace", extra_exclude=None, allow_fetch=False, live_fetch=False):
    ...
    if section == "items":
        ...
        return store, {
            ...
            "running": False,
            "index": index,
            "message": "مسار الاستبدال الجديد غير متاح، ولم يتم استخدام الجينريتور القديم.",
            "refill": failed,
            "refill_stages": stages,
        }
    ...
    return store, {
        ...
        "running": False,
        "index": index,
        "message": "لم يتم العثور على بديل مناسب من المسار الجديد، ولم يتم استخدام الجينريتور القديم.",
        "refill": failed,
        "refill_stages": stages,
    }
